...
function GetIgnoreChannels()
	local icList = ""
	local iNext = 0
	if GlobalIgnoreDB.ignoreChannels ~= nil then
		for i = 1, #GlobalIgnoreDB.ignoreChannels do
			if GlobalIgnoreDB.ignoreChannels[i] ~= nil then
				icList = icList .. GlobalIgnoreDB.ignoreChannels[i]
			end
			iNext = i + 1
			if iNext <= #GlobalIgnoreDB.ignoreChannels and GlobalIgnoreDB.ignoreChannels[iNext] ~= nil then
				icList = icList .. ";"
			end
		end
	end
...
function SetIgnoreChannels(icList)
	GlobalIgnoreDB.ignoreChannels = {}
	for chname in string.gmatch(icList, "[^;]+") do
		if chname ~= nil and string.len(chname) then table.insert(GlobalIgnoreDB.ignoreChannels, chname) end
	end		
end
...
local function CreateUIFrames()
	...
	Button:SetPoint("TOPLEFT", Text, "BOTTOMLEFT", 30, 0)
	...
	Button:SetScript("OnEscapePressed", function(self) self:SetText(GetIgnoreChannels()) end)
	Button:SetScript("OnEnterPressed",  function(self) SetIgnoreChannels(self:GetText()) self:ClearFocus() end)
	Button:SetScript("OnEnter",
		function(self)
			GameTooltip:SetOwner(self)
			GameTooltip:ClearLines()
			GameTooltip:AddLine(L["TIP_3"]);
			GameTooltip:Show()
		end)
	Button:SetScript("OnLeave",
		function(self)
