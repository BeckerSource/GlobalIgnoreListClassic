...
local function chatMessageFilter (self, event, message, from, t1, t2, t3, t4, t5, chnum, chname, ...)
	...
	--	print ("chatMsg chname=" .. (chname or "nil") .. " chnum=" .. (chnum or "nil"))
	--	lastMsg = message
	--end
	...
	elseif (from ~= nil) and (from ~= "") then
		...
		if (hasGlobalIgnored(from) > 0 or hasServerIgnored(getServer(from)) > 0) and IsIgnoreChannelEvent(chname, event) == true then
...
function IsIgnoreChannelEvent(chname, event)
	...
	for i = 1, #GlobalIgnoreDB.ignoreChannels do
		-- print("DEBUG: chname=" .. chname .. ", event=" .. event .. ", i=" .. i .. ", ic=" .. GlobalIgnoreDB.ignoreChannels[i])
		if GlobalIgnoreDB.ignoreChannels[i] ~= nil and string.len(GlobalIgnoreDB.ignoreChannels[i]) and
			(
			 (chname and chname ~= '' and string.lower(GlobalIgnoreDB.ignoreChannels[i]) == string.lower(chname)) or
			 (event  and event  ~= '' and string.lower(GlobalIgnoreDB.ignoreChannels[i]) == string.lower(event))
			) 
			then return true
		end
	end
